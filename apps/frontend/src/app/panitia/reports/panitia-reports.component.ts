...
@Component({
  selector: 'app-panitia-reports',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="page-header" style="margin-bottom: 1.5rem;">
      <h2>Laporan Event yang Saya Kelola (Panitia)</h2>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
      ...
      <div class="stat-card">
        <h3>{{ summary()?.totalParticipants || 0 }}</h3>
        <p>Total Pendaftar</p>
      </div>
      <div class="stat-card">
        <h3>{{ summary()?.totalValidated || 0 }}</h3>
        <p>Disetujui</p>
      </div>
      <div class="stat-card">
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr *ngIf="events().length === 0">
            <td colspan="4" style="text-align: center; padding: 2rem; color: var(--text-light);">Belum ada event yang ditugaskan ke Anda.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="modal-backdrop" *ngIf="isModalOpen()">
      <div class="modal-content" style="background: white; padding: 2rem; border-radius: 8px; width: 100%; max-width: 800px; margin: 5vh auto; position: relative;">
        <h3>Laporan Detail: {{ reportData()?.event?.title }}</h3>
        ...
        <!-- Participants Table -->
        <h4>Daftar Peserta & Kehadiran</h4>
        <div style="max-height: 250px; overflow-y: auto; border: 1px solid var(--border-color); border-radius: 4px; margin-top: 0.75rem;">
          <table class="table" style="margin-bottom: 0;">
            <thead>
              <tr style="text-align: left; background: #f8fafc; border-bottom: 1px solid var(--border-color);">
                ...
                <th style="padding: 0.5rem;">Kehadiran</th>
...
export class PanitiaReportsComponent implements OnInit {
  summary = signal<PanitiaSummary | null>(null);
  events = signal<Event[]>([]);
  ...
  loadSummary() {
    this.http.get<{ success: boolean; data: PanitiaSummary }>(`${environment.apiUrl}/reports/panitia/summary`).subscribe({
      next: (res) => {
